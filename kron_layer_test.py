...
from kron_layer import KronLayer, OldKronLayer
...
def build_custom_cnn(input_var=None, widths=None, drop_input=0.2,
                     drop_hidden=.5, type="dense", params=None):
    # By default, this creates the same network as `build_mlp`, but it can be
    # customized with respect to the number and size of hidden layers. This
    # mostly showcases how creating a network in Python code can be a lot more
    ...
    if type == "dense":
        ...
        param_density = params.get('param_density', 1.0)
        shape2 = params.get('shape2', (28, 200))
...
def build_custom_cnn(input_var=None, widths=None, drop_input=.2,
    ...
    if type == "dense":
        ...
        network = KronLayer(network, widths[0], mode=mode, param_density=param_density, name="kron_fixedrank0")
...
def comparison(X_train,y_train,X_val,y_val,X_test,y_test, kron_params=None):
    ...
    trains, accs = list(zip(*([(trains, accs)]
                              ...
                              + [generate_train_acc(widths=hidden_units, type="old_kron", params=kron_param) for kron_param in kron_params])))

    names = ["dense"] + ["kron({})".format(p.values()) for p in kron_params] + ["old_kron({})".format(p.values()) for p in kron_params]
    results = {}
